// Port of AVL limits.f with float32 math for numerical fidelity.
...
export function GLIMS(state, XYZMIN = new Float32Array(3), XYZMAX = new Float32Array(3), LPROJ = false) {
  ...
  if (state.LOBPLT) {
    for (let n = 0; n < state.NOB; n += 1) {
      lset = true;
      pts[idx2(0, 0, 3)] = state.ROB[idx2(0, n, 3)];
      pts[idx2(1, 0, 3)] = state.ROB[idx2(1, n, 3)];
      pts[idx2(2, 0, 3)] = state.ROB[idx2(2, n, 3)];
      if (LPROJ) {
        VIEWPROJ(state, pts, 1, pts);
      }
      for (let k = 0; k < 3; k += 1) {
        const v1 = pts[idx2(k, 0, 3)];
        XYZMIN[k] = Math.min(XYZMIN[k], v1);
        XYZMAX[k] = Math.max(XYZMAX[k], v1);
      }
    }
  }
