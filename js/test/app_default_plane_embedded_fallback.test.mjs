...
test('boot uses embedded plane.avl when fetch fails', { timeout: 30000 }, async () => {
  ...
  const defaultPlane = { textContent: planeText };
  const elMap = {
    appRoot: makeEl(),
    navSettings: makeEl(),
    navPlots: makeEl(),
  ...
  const context = {
    ...
    document: {
      getElementById: (id) => {
        if (id === 'defaultPlane') return defaultPlane;
        return elMap[id] ?? makeEl();
      },
      querySelectorAll() { return []; },
      createElement() {
        return {
  ...
  assert.ok(debugLogs.some((line) => line.includes('Loaded embedded default file: plane.avl')));
});
