...
test('BDFORC WASM matches Fortran reference', async (t) => {
  ...
  const srcU = new Float32Array(NLNODE * NUMAX);
  for (let iu = 0; iu < NUMAX; iu += 1) {
    srcU[idx2(0, iu, NLNODE)] = Math.fround(0.01 * (iu + 1));
    srcU[idx2(1, iu, NLNODE)] = 0.0;
  }
  ...
  const state = {
    ...
    LFRST: Int32Array.from([0]),
    RL: Float32Array.from([0.0, 0.0, 0.0, 1.0, 0.5, 0.2]),
    RADL: Float32Array.from([0.10, 0.12]),
    SRC: Float32Array.from([0.8, 0.0]),
    SRC_U: srcU,
    ...
    DCPB: new Float32Array(3 * NLNODE),
  ...
  const { BDFORC } = await loadAeroWasm();
  BDFORC(state);
  ...
  for (let l = 0; l < 3; l += 1) out.push(state.DCPB[idx2(l, 0, 3)]);
