...
function ensureRefBuilt() {
  const proc = spawnSync('make', ['-B', 'b737_exec_ref'], { cwd: refDir, encoding: 'utf8' });
...
async function runB737Exec(niter) {
...
test('EXEC matches Fortran for b737.avl forces and trefftz', { timeout: 140000 }, async () => {
  ensureRefBuilt();

  const refProc = spawnSync(refBin, ['b737.avl', '0'], { encoding: 'utf8', cwd: runsDir });
  ...
  const state = await runB737Exec(0);
...
test('EXEC matches Fortran for b737.avl with iterations', { timeout: 200000 }, async () => {
  ensureRefBuilt();

  const refProc = spawnSync(refBin, ['b737.avl', '3'], { encoding: 'utf8', cwd: runsDir });
  if (refProc.error) throw refProc.error;
  if (refProc.status !== 0) {
    throw new Error(refProc.stderr || refProc.stdout || `ref exited with ${refProc.status}`);
  }
  ...
  const state = await runB737Exec(3);
