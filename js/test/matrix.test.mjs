...
import { loadMatrixWasm } from '../src/matrix_wasm.js';

const repoRoot = path.resolve(path.dirname(new URL(import.meta.url).pathname), '..', '..');
const refDir = path.join(repoRoot, 'third_party', 'avl', 'ref');
const wasmPath = path.join(repoRoot, 'js', 'dist', 'matrix.wasm');
...
test('LUDCMP/BAKSUB JS matches Fortran reference', async (t) => {
...
test('LUDCMP/BAKSUB WASM matches Fortran reference', async (t) => {
  const refBin = path.join(refDir, 'matrix_ref');
  if (!fs.existsSync(refBin)) {
    t.skip(`Fortran reference binary not found: ${refBin}`);
    return;
  ...
  const ref = runFortranRef(cases);
  const { solve } = await loadMatrixWasm({ wasmPath });
  cases.forEach((c, i) => {
