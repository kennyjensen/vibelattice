...
import { TPFORC } from '../src/atpforc.js';
...
test('TPFORC JS matches Fortran reference', async (t) => {
  ...
  const RV1 = new Float32Array(4 * (NSTRIP + 1));
  const RV2 = new Float32Array(4 * (NSTRIP + 1));
  const RC = new Float32Array(4 * (NSTRIP + 1));
  ...
  const state = {
    ...
    IJFRST: Int32Array.from([0, 1, 2]),
    NVSTRP: Int32Array.from([0, 1, 1]),
    GAM: Float32Array.from([0.0, 0.5, 0.3]),
    GAM_U: (() => {
      const out = new Float32Array((NSTRIP + 1) * NUMAX);
      for (let i = 1; i <= 2; i += 1) {
        for (let n = 0; n < NUMAX; n += 1) {
          out[i * NUMAX + n] = Math.fround(0.05 * (i + n + 1));
        }
      }
      return out;
    })(),
    GAM_D: new Float32Array(0),
    GAM_G: new Float32Array(0),
    RV1,
    RV2,
    RC,
    CHORD: Float32Array.from([0.0, 1.0, 0.8]),
    LSSURF: Int32Array.from([0, 1, 1]),
    LNCOMP: Int32Array.from([0, 1]),
    LFLOAD: [false, true],
    LSTRIPOFF: new Uint8Array(NSTRIP + 1),
  };

  const out = TPFORC(state);
  const scalars = [out.CLFF, out.CYFF, out.CDFF, out.SPANEF];

  assertCloseArray(scalars, refScalars);
  assertCloseArray(Array.from(out.DWWAKE.slice(1)), refDwwake);
  assertCloseArray(Array.from(out.CLFF_U), refClffU);
  assertCloseArray(Array.from(out.CYFF_U), refCyffU);
  assertCloseArray(Array.from(out.CDFF_U), refCdffU);
  assertCloseArray(Array.from(out.SPANEF_U), refSpanU, 1e-4);
});
