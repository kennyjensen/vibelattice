...
for (const entry of entrypoints) {
  test(`Total Forces alpha/beta use 3 decimals on ${entry}`, async ({ page }) => {
    ...
    try {
      ...
      await expect.poll(async () => (await page.locator('#outAlpha').innerText()).trim(), {
        timeout: 30000,
      }).toMatch(/^-?\d+\.\d{3}$/);
      await expect.poll(async () => (await page.locator('#outBeta').innerText()).trim(), {
        timeout: 30000,
      }).toMatch(/^-?\d+\.\d{3}$/);
    } finally {
