...
for (const entry of entrypoints) {
  test(`hershey body-axis CZq matches Fortran sign on ${entry}`, async ({ page }) => {
    ...
    try {
      ...
      const czqCell = page.locator('#outBodyDeriv .stability-cell[title="CZq"] .stability-num');
      await expect(czqCell).not.toHaveText(/^\s*-\s*$/);
      const czq = parseSigned(await czqCell.innerText());
