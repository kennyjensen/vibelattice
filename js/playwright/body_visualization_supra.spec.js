...
test('supra body is rendered in 3D visualization', async ({ page }) => {
  ...
  try {
    ...
    const summary = await page.evaluate(() => window.__trefftzTestHook?.getBodyVisualState?.() || null);
    expect(Array.isArray(summary?.requiredBodyFiles)).toBeTruthy();
    expect(summary.requiredBodyFiles.includes('fuseSupra.dat')).toBeTruthy();
  } finally {
