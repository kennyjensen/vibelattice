(module
  ...
  (func (export "LUDCMP") (param $nsiz i32) (param $n i32) (param $a i32) (param $indx i32) (param $work i32)
    ...
    (local.set $one (f32.const 1))

    ;; WORK(i) = 1.0 / max(abs(A(i,j)))
      ...
      (loop $i_loop0
        ...
        (block $j_done0
          (loop $j_loop0
            ...
            (local.set $sum (f32.abs (f32.load (local.get $idx))))
        ...
        (local.set $idx (i32.add (local.get $work) (i32.mul (local.get $i) (i32.const 4))))
        (f32.store (local.get $idx) (f32.div (local.get $one) (local.get $aamax)))
      ...
      (loop $j_loop1
        ...
        (block $i_done1
          (loop $i_loop1
            ...
            (local.set $addr_ij
              (i32.add (local.get $a)
                       (i32.mul (i32.add (i32.mul (local.get $i) (local.get $n)) (local.get $j)) (i32.const 4))))
            (local.set $sum (f32.load (local.get $addr_ij)))
              ...
              (loop $k_loop1
                ...
                (local.set $sum
                  (f32.sub (local.get $sum)
                           (f32.mul (f32.load (local.get $addr_ik)) (f32.load (local.get $addr_kj)))))
                (local.set $k (i32.add (local.get $k) (i32.const 1)))
                (br $k_loop1)
              )
            )
            (f32.store (local.get $addr_ij) (local.get $sum))
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (br $i_loop1)
          )
        )
        ...
        (block $i_done2
          (loop $i_loop2
            ...
            (local.set $idx
              (i32.add (local.get $work) (i32.mul (local.get $i) (i32.const 4))))
            (local.set $dum (f32.mul (f32.load (local.get $idx)) (f32.abs (local.get $sum))))
          ...
          (then
            ...
            (block $k_done3
              (loop $k_loop3
                ...
                (local.set $idx
                  (i32.add (local.get $a)
                           (i32.mul (i32.add (i32.mul (local.get $imax) (local.get $n)) (local.get $k)) (i32.const 4))))
                (local.set $idx2
                  (i32.add (local.get $a)
                           (i32.mul (i32.add (i32.mul (local.get $j) (local.get $n)) (local.get $k)) (i32.const 4))))
                (local.set $dum (f32.load (local.get $idx)))
                (f32.store (local.get $idx) (f32.load (local.get $idx2)))
                (f32.store (local.get $idx2) (local.get $dum))
                (local.set $k (i32.add (local.get $k) (i32.const 1)))
                (br $k_loop3)
              )
            )
            (local.set $idx
              (i32.add (local.get $work) (i32.mul (local.get $imax) (i32.const 4))))
            (local.set $idx2
              (i32.add (local.get $work) (i32.mul (local.get $j) (i32.const 4))))
            (local.set $dum (f32.load (local.get $idx)))
            (f32.store (local.get $idx) (f32.load (local.get $idx2)))
            (f32.store (local.get $idx2) (local.get $dum))
          )
        ...
        (if (i32.ne (local.get $j) (i32.sub (local.get $n) (i32.const 1)))
          (then
            (local.set $idx
              (i32.add (local.get $a)
                       (i32.mul (i32.add (i32.mul (local.get $j) (local.get $n)) (local.get $j)) (i32.const 4))))
            (local.set $dum (f32.div (local.get $one) (f32.load (local.get $idx))))
            ...
            (block $i_done3
              (loop $i_loop3
                ...
                (local.set $idx
                  (i32.add (local.get $a)
                           (i32.mul (i32.add (i32.mul (local.get $i) (local.get $n)) (local.get $j)) (i32.const 4))))
                (f32.store (local.get $idx) (f32.mul (f32.load (local.get $idx)) (local.get $dum)))
                (local.set $i (i32.add (local.get $i) (i32.const 1)))
                (br $i_loop3)
              )
            )
  ...
  (func (export "BAKSUB") (param $nsiz i32) (param $n i32) (param $a i32) (param $indx i32) (param $b i32)
    ...
    (local $sum f32) (local $idx i32) (local $idx2 i32) (local $addr_bi i32)
    ...
    (block $i_done
      (loop $i_loop
        ...
        (local.set $idx (i32.add (local.get $b) (i32.mul (local.get $ll) (i32.const 4))))
        (local.set $sum (f32.load (local.get $idx)))
        (local.set $addr_bi (i32.add (local.get $b) (i32.mul (local.get $i) (i32.const 4))))
        (f32.store (local.get $idx) (f32.load (local.get $addr_bi)))
              ...
              (loop $j_loop
                ...
                (local.set $idx
                  (i32.add (local.get $a)
                           (i32.mul (i32.add (i32.mul (local.get $i) (local.get $n)) (local.get $j)) (i32.const 4))))
                (local.set $idx2
                  (i32.add (local.get $b) (i32.mul (local.get $j) (i32.const 4))))
                (local.set $sum
                  (f32.sub (local.get $sum)
                           (f32.mul (f32.load (local.get $idx)) (f32.load (local.get $idx2)))))
                (local.set $j (i32.add (local.get $j) (i32.const 1)))
                (br $j_loop)
              )
            )
        ...
        (f32.store (local.get $addr_bi) (local.get $sum))
        (local.set $i (i32.add (local.get $i) (i32.const 1)))
        (br $i_loop)
      )
    )
    ...
    (block $i_done2
      (loop $i_loop2
        ...
        (local.set $addr_bi (i32.add (local.get $b) (i32.mul (local.get $i) (i32.const 4))))
        (local.set $sum (f32.load (local.get $addr_bi)))
        ...
        (local.set $idx
          (i32.add (local.get $a)
                   (i32.mul (i32.add (i32.mul (local.get $i) (local.get $n)) (local.get $i)) (i32.const 4))))
        (f32.store (local.get $addr_bi) (f32.div (local.get $sum) (f32.load (local.get $idx))))
