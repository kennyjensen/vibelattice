...
(module
  ...
  (func (export "VORVEL")
    ...
    (local $adb f32) (local $den f32) (local $t f32)
    ...
    (local.set $asq
      (f32.add
        (f32.add (f32.mul (local.get $a1) (local.get $a1))
                 (f32.mul (local.get $a2) (local.get $a2)))
        (f32.mul (local.get $a3) (local.get $a3))))

    (local.set $bsq
      (f32.add
        (f32.add (f32.mul (local.get $b1) (local.get $b1))
      ...
      (then
        (if (f32.ne (f32.mul (local.get $amag) (local.get $bmag)) (f32.const 0))
          (then
            ...
            (local.set $adb
              (f32.add
                (f32.add (f32.mul (local.get $a1) (local.get $b1))
                         (f32.mul (local.get $a2) (local.get $b2)))
                (f32.mul (local.get $a3) (local.get $b3))))

            (local.set $den (f32.add (f32.mul (local.get $amag) (local.get $bmag)) (local.get $adb)))

            (if (f32.ne (local.get $den) (f32.const 0))
              (then
                (local.set $t
                  (f32.div
                    (f32.add (f32.div (f32.const 1) (local.get $amag))
                             (f32.div (f32.const 1) (local.get $bmag)))
                    (local.get $den)))
    ...
    (if (f32.ne (local.get $amag) (f32.const 0))
      (then
        (local.set $axisq
          (f32.add (f32.mul (local.get $a3) (local.get $a3))
                   (f32.mul (local.get $a2) (local.get $a2))))
    ...
    (if (f32.ne (local.get $bmag) (f32.const 0))
      (then
        (local.set $axisq
          (f32.add (f32.mul (local.get $b3) (local.get $b3))
                   (f32.mul (local.get $b2) (local.get $b2))))
        (local.set $rsq (local.get $axisq))
        (local.set $t
          (f32.div
